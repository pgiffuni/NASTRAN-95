...
!*****
! THIS ROUTINE GENERATES THE GRID POINT SINGULARITY TABLE BY EXAMINING
! THE TRANSLATIONAL AND DIAGONAL 3 X 3 SUBMATRICES OF THE KGG MATRIX.
...
!*****
 ...
 EQUIVALENCE (iz(1),dz(1))      ,(iarray(1),iorder)  &
     ,                  (iarray(2),nwds)
...
! INSURE THE SYMMETRY OF THE B MATRIX
 ...
 IF (b(2) /= 0.0D0 .AND. b(4) /= 0.0D0) GO TO 82
 b(2) = 0.0D0
 b(4) = 0.0D0
 GO TO 83
 ...
 b(2) = temp
 b(4) = temp
 83 IF (b(3) /= 0.0D0 .AND. b(7) /= 0.0D0) GO TO 84
 b(3) = 0.0D0
 b(7) = 0.0D0
 GO TO 85
 ...
 b(3) = temp
 b(7) = temp
 85 IF (b(6) /= 0.0D0 .AND. b(8) /= 0.0D0) GO TO 86
 b(6) = 0.0D0
 b(8) = 0.0D0
 GO TO 90
...
! AT THIS POINT ONE AND ONLY ONE FL(I) IS ZERO.
 ...
 170 IF( fm == 0.0D0 ) GO TO 171
 IF( fr <= 0.0D0 ) GO TO 240
 IF ( fm/fr >= dtol ) GO TO 240
...
! AT STATEMENT NO. 250, WE HAVE THAT ALL THE FL(I) ARE .GT. 0.0D0, SO
