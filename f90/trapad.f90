...
!     LSYS78 = .TRUE. IF KSYS78 = 0, OR 2
 ...
 DOUBLE PRECISION :: bmass(12,12),bmbss(144),acurl(208),d(144),      &
                     ak(144),akj(256),akt(27),pi,twopi,xq,degrad,    &
                     e(3),anu(3),gb(12,12),r(4),z(4),delint(12),     &
                     ee(63),teo(45),sp(36),zmin,gamr,cosg,sing,v,vz, &
                     vr,del,c2s2,c2,s2,c4,s4,dgam,ajho,ajjho,rhod,   &
                     ar,rzintd,rmin,rmax,d1(48),d2(16),gbp(4,4),     &
                     acurp1(48),acurp2(16),akuph(48),akph2(16)
...
!     INPUT AND OUTPUT BLOCKS FOR SUBROUTINE MAT
 ...
 COMMON /matout/  es(3),anus(3),rho,g(3),alf(3),tzero,gsube, moskp(9),setmat
...
!     FATAL IF RATIO OF RADII IS TO LARGE FOR GUASS QUADRATURE
 ...
 IF (rmax/rmin <= 10.d0) GO TO 206
 IF (rmax/rmin > 10.d0) GO TO 7760
 
 206 IF (r(1) >= r(2) .OR. r(4) >= r(3) .OR. z(4) <= z(1)) GO TO 7770
 IF (DABS(z(1)-z(2)) > 1.0D-3) GO TO 7770
 IF (DABS(z(3)-z(4)) > 1.0D-3) GO TO 7770
 d(5) = (r(1)+r(4))/2.d0
 d(6) = (r(2)+r(3))/2.d0
 IF (d(5) == 0.d0) GO TO 210
 IF (DABS((r(1)-r(4))/d(5)) > .5D-2) GO TO 210
 r(1) = d(5)
 r(4) = d(5)
 210 CONTINUE
 IF (d(6) == 0.d0) GO TO 220
 IF (DABS((r(2)-r(3))/d(6)) > .5D-2) GO TO 220
 r(2) = d(6)
 r(3) = d(6)
 220 CONTINUE
