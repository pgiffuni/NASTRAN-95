...
!     ECPT(42)  -  ELTEMP        AVG. ELEMENT TEMPERATURE
 ...
 REAL, INTENT(OUT)                        :: z(1)
 INTEGER, INTENT(IN OUT)                  :: idefm
 INTEGER, INTENT(IN OUT)                  :: nogptt
 INTEGER, INTENT(IN OUT)                  :: noedt
 LOGICAL :: abasic   ,bbasic   ,basic    ,aofset ,bofset   , offset
 REAL :: l        ,lsq      ,lcube    ,i1     ,i2       ,  &
     k1       ,k2       ,ke       ,kep    ,i12      ,  &
     nsm      ,lr1      ,lr2      ,lb     ,l2b3     , l2b6     ,ua(6)
 DIMENSION         veci(3)  ,vecj(3)  ,veck(3)  , ta(18)   ,  &
     tb(9)    ,ecpt(100),iecpt(38),ipin(10)         , smalv0(6)
 
!     SDR2 PHASE I INPUT AND OUTPUT COMMON BLOCK
 
 COMMON   /trimex/ ielid    ,isilno(2),smallv(3),icssv  ,ipinfl(2),  &
     za(3)    ,zb(3)    ,imatid   ,a      ,i1       ,  &
     i2       ,fj       ,nsm      ,fe     ,c1       ,  &
     c2       ,d1       ,d2       ,f1     ,f2       ,  &
     g1       ,g2       ,k1       ,k2     ,i12      ,  &
     mcsida   ,gpa(3)   ,mcsidb   ,gpb(3) ,tempel
...
!     INPUT AND OUTPUT BLOCKS FOR SUBROUTINE MAT
 ...
 COMMON   /matout/ e        ,g        ,nu       ,rho     ,alpha   ,  &
     t_sub_0  ,g_sub_e  ,sigt     ,sigc    ,sigs
...
!     NOW PERFORM THE ELEMENT TEMPERATURE AND DEFORMATION LOADING.
 ...
 500 tbar = tsub0
 IF (nogptt == 0) GO TO 510
 CALL ssgetd (ecpt(1),ke(1),0)
 tbar = (ke(1) + ke(2))/2.0
 510 delta = 0.0
 IF (noedt == 0) GO TO 520
 ke(3) =  0.0
 ke(4) =  0.0
 ke(5) =  0.0
 ke(6) =  0.0
