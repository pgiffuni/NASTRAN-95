...
!     PERFORMS THE TRIDIAGONAL REDUCTION FOR THE COMPLEX FEER METHOD
 ...
 REAL, INTENT(OUT)                        :: v1l(1)
 REAL, INTENT(IN OUT)                     :: v2(1)
 REAL, INTENT(IN OUT)                     :: v2l(1)
 REAL, INTENT(IN)                         :: v3(1)
 REAL, INTENT(IN)                         :: v3l(1)
 REAL, INTENT(OUT)                        :: v4(1)
 REAL, INTENT(OUT)                        :: v4l(1)
 REAL, INTENT(IN OUT)                     :: v5(1)
 REAL, INTENT(IN OUT)                     :: v5l(1)
 REAL, INTENT(IN OUT)                     :: zb(1)
 REAL, INTENT(IN OUT)                     :: zc(1)
 LOGICAL :: sucess   ,no_b     ,skip     ,again      , qpr      ,symmet
 INTEGER :: csp      ,NAME(2)
 REAL :: lambda   ,temp1(2)
...
!     COMPRESS PHYSICAL EIGENVECTORS TO SINGLE PRECISION
   ...
   IF (idiag /= 0 .AND. nord2 <= 70) WRITE (nout,570) i,  &
       (v1(j),v1(j+1),v1l(j),v1l(j+1),j=1,nord2,2)
...
!     THIS LOADS VALUES INTO V1 AND V1L
   ...
   30 v1(jj) = ss*(MOD(ij,3)+1)/ (3.*(MOD(ij,13)+1)*(1+5*FLOAT(i)/nord))
 END DO
...
!     NULL VECTOR TEST
 ...
 IF (SQRT(d(1)**2+d(2)**2) > SQRT(a(1)**2+a(2)**2)*tenmht) GO TO 100
...
!     NULL VECTOR TEST
 ...
 IF (SQRT(d(1)**2+d(2)**2) <= SQRT(a(1)**2+a(2)**2)*tenmht) GO TO 160
 GO TO 110
 105 again = .false.
 d(1)  = zero
 d(2)  = zero
...
!     TRANSFER TWO ELEMENTS TO REDUCED TRIDIAGONAL MATRIX FILE
 ...
 600 FORMAT (a25,' 3158', //5X,'NO ADDITIONAL MODES CAN BE FOUND BY ',  &
     'FEER IN THE NEIGHBORHOOD OF ',2E14.6,//)
