SUBROUTINE alg04(h,s,vw,r1,r2,x1,x2,vm,eps,sclfac,g,ej,hmin,vmin,  &
 ...
 DIMENSION vzup(21),vzdn(21),psdn(21),hup(21),vwup(21),sup(21), &
           rmid(20),delr(20),psup(21),hsup(21),vwfun(21),vzfun(21), &
           sdn(21),hsdn(21),vwdn(21),hdn(21),xx1(21),xx2(21),r(21)
 ...
 flow=0.0
 DO  j=1,itub
   flow=flow+(r(j+1)**2-r(j)**2)*(vzup(j)+vzup(j+1))*alg5((hsup(j) &
            +hsup(j+1))*0.5,(sup(j)+sup(j+1))*0.5)
 END DO
 ...
 GO TO 350
 400   DO  j=1,nstrms
   vzdn(j)=vzup(j)+(vzfun(j)-(psdn(j)-psup(j)) &
          /alg5(hsup(j),sup(j))*g )/vzup(j)
 ...
 xx1(1)=0.0
 DO  j=1,itub
   xx1(j+1)=xx1(j)+(r(j+1)**2-r(j)**2)*(vzdn(j+1)+vzdn(j)) &
           *alg5((hsdn(j)+hsdn(j+1))*0.5,(sdn(j)+sdn(j+1))*0.5)
 END DO
 q1=xx1(nstrms)
