...
!     WILKINSON EIGENVECTOR SOLUTION FOR LARGE SYM MATRICES
 ...
 REAL :: value,w,x,y,z,dlmdas, rmult,rrmult,sft,sftinv,deps,vmult,zero,one
...
!     OF TRIAL VECTOR AND ORTHOGONOLIZATION COMPUTATIONS
 ...
 iclos = 0
 i = nrigid + 1
 10 IF (ABS(val(i))+ABS(val(i+1)) < rmult) GO TO 20
 IF (val(i) == zero) GO TO 90
 IF (ABS(one-val(i)/val(i+1)) > rrmult) GO TO 80
...
!     SEARCH FOR MULTIPLICITIES OF EIGENVALUES V2 AND V2+1.
 ...
 102 vv = v2
 103 IF (ABS(val(v2))+ABS(val(v2+1)) < rmult) GO TO 1041
 IF (ABS(one-val(v2)/val(v2+1))   > rmult) GO TO 110
 1041 CONTINUE
 l1 = vloc(v2  )
 l2 = vloc(v2+1)
 n1 = MIN0(l1,l2)
...
!     SET UP SIMULTANEOUS EQUATIONS
   ...
   DO  k = n1,n2m1
     ...
     GO TO 126
     125 f(k) = -sft*o(k)
     126 IF (ABS(x)-ABS(o(k)) < 0.0) THEN
       GO TO   127
     ELSE IF (ABS(x)-ABS(o(k)) == 0.0) THEN
       GO TO   128
     ELSE
       GO TO   129
     END IF
...
!     IN THE FOLLOWING CHECKS
   ...
   IF (ABS(val(vector+1))+ABS(val(vector)) < rmult) GO TO 154
   IF (ABS(val(vector+1)-val(vector)) > rmult*ABS(val(vector+1))) GO TO 160
   154 CONTINUE
   l1 = vloc(vector+1)
