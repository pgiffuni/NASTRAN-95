...
!      45          R    ELEMENT TEMPERATURE
 ...
 REAL :: amgg(1),akgg,dgpth(4),bmat1(384),xybmat(96),zeta,  &
     mominr,vol,voli,th,area,area2,detj,ptint(2),  &
     eps1,xi,eta,zta,hzta,thk,xmasso,v(3,3),  &
     coeff,xmtmp(16),xmass(16),tmpmas(9),jacob(3,3),  &
     tmpshp(4),tmpthk(4),dshptp(8),psitrn(9),phi(9),  &
     shp(4),dshp(8),tgrid(4,4),colstf(144),trans(36),  &
     trans1(36),coltmp(144),avgthk,temp
 ...
 REAL :: rho,ts,tsi,reali,rhox,thetam,xm,ym,u(9),a,b,  &
     aspect,thlen,xa(4),yb(4),GT(9),gi(36),enorx,enory,  &
     gnorx,gnory,nunorx,nunory,dsub,dsub4,psiinx,  &
     psiiny,tsmfx,tsmfy,curvtr(3,4),curve(3),sineax,  &
     sineay,w1,pi,twopi,raddeg,degrad,htflx(12),determ,  &
     htcap(16),htcon(16),dvol,dheat,weitc,bterms(32)
 REAL :: zc(4),uev,anglei,edgel,edgshr,unv,vnt(3,4),aspctx,  &
     aspcty,gfour(10,10),dfour(7,7),bfour(240),csubb4,  &
     csubx,csuby,csubt,csubtx,csubty,sfctr1,sfctr2,  &
     sfctx1,sfctx2,sfcty1,sfcty2,offset,const
...
!     NCORE = CURRENT LAST AVAILABLE LOCATION IN OPEN CORE
 ...
 COMMON /emgprm/ icore,jcore,ncore,icstm,ncstm,imat,nmat,ihmat,  &
     nhmat,idit,ndit,icong,ncong,lcong,anycon,flags(3),  &
     precis,error,heat,cpmass,lcstm,lmat,lhmat, kflags(3),l38
...
!    1                (SYS(50)   ,ICSUBT  ), (SYS(75) ,ICSUB8    )
 EQUIVALENCE     (flags(1)  ,kgg1    ), (flags(2),mgg1      ),  &
     ...
     (amgg(1)   ,akgg(1) ), (necpt(1),ecpt(1)   ),  &
     (htcp      ,kheat(4)), (htflx(1),tmpmas(1) ),  &
     (htcap(1)  ,xmass(1)), (htcon(1),xmtmp(1)  ),  &
     (nsm       ,est(19) ), (matset  ,matout(25)), (iegpdt(1,1),egpdt(1,1)),  &
     (igpdt(1,1) ,bgpdt(1,1))
 DATA    eps1  / 1.0E-7 /
...
!     STORE THE CORNER NODE DIFF. IN THE USER C.S.
 ...
 IF (aa == 0.0 .OR. bb == 0.0) GO TO 1700
...
!     SET IDENTT FLAG TO 1 IF TEB IS AN IDENTITY MATRIX
 ...
 identt = 0
 IF (teb(1) == 1.0 .AND. teb(5) == 1.0 .AND. teb(9) == 1.0 .AND.  &
     teb(2) == 0.0 .AND. teb(3) == 0.0 .AND. teb(4) == 0.0 .AND.  &
     teb(6) == 0.0 .AND. teb(7) == 0.0 .AND. teb(8) == 0.0 ) identt = 1
 ip = -3
 DO  ii = 2,4
   ip = ip + 3
   DO  j = 1,nnode
...
!     MATERIAL C.S. ON TO THE XY PLANE OF THE ELEMENT C.S.
 ...
 xm = tem(1)
 ym = tem(4)
 IF (ABS(xm) > eps1 .OR. ABS(ym) > eps1) GO TO 460
 nest(2) = mcsid
 j = 231
 GO TO 1720
...
!     END OF M-LOOP
 ...
 IF (gi(19) /= 0.0 .OR. gi(20) /= 0.0 .OR. gi(21) /= 0.0 .OR.  &
     gi(22) /= 0.0) GO TO 800
 igobk = 1
 m = 2
 mid(3) = mid(2)
...
!     ADD NON-STRUCTURAL MASS
 ...
 IF (rho == 0.0 .AND. nsm == 0.0) GO TO 1410
!     IF (CPMASS .GT. 0) GO TO 1410
 IF (nsm  == 0.0) GO TO 1410
 IF (vol == 0. .OR. rhox == 0.) WRITE (nout,2060) sfm,elid,area,  &
     vol,rhox,mgg1,kgg1
...
!     AND MATRICES.
   ...
   1420 IF (identt /= 1 .OR. offset /= 0.0) GO TO 1430
   notran(i) = 1
   CYCLE
...
!     OMMENT . ANIS = .FALSE. MEANS ISOTROPIC THERMAL CONDUCTIVITY.
 ...
 2060 FORMAT (a25,', ZERO VOLUME OR DENSITY FOR QUAD4 ELEMENT ID =',i9,  &
     ', AREA,VOL,RHO=',3E12.3, /70X,'MGG1,KGG1=',2I8)
