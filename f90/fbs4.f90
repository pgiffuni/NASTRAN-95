...
!     FBS4 EXECUTES THE FORWARD/BACKWARD PASS FOR FBSF IN CDP
 ...
 DOUBLE PRECISION, INTENT(IN OUT)         :: yn(1)
 INTEGER, INTENT(IN)                      :: nwds
INTEGER :: dbl, buf(2), subnam, begn, END
DOUBLE PRECISION :: ljjr, ljji, l, yjkr, yjki, ssqr, sum1, sum2,  zero
...
DO  j = 1,n
  ...
  DO  k = 1,last,nbritm
    yjkr = y(j1*2+k  )
    yjki = y(j1*2+k+1)
    IF (yjkr /= zero .OR. yjki /= zero) GO TO 7
  END DO
...
!     LOAD VECTOR IN CORE -- Y(I,K) = Y(I,K) + L(I,J)*Y(J,K)
  ...
  10 DO  k = 1,last,nbritm
    yjkr = y(j1*2+k  )
    yjki = y(j1*2+k+1)
    IF (yjkr == zero .AND. yjki == zero) CYCLE
    ik = 2*BLOCK(4) + k - 2
    DO  ij = jstr,nstr,2
