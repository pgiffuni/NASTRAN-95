...
!     SKIP THE TRANSFORMATION OF ALPHAM IF MATSET = 1. OR THETAM = 0.D0
 ...
 640 IF (matset ==   1.0) GO TO 650
 IF (thetam /= 0.0D0) GO TO 670
...
!     CALCULATE THERMAL FORCES AND MOMENTS
   ...
   DO  ir = 1,3
     ...
     ftherm(ir+3) = ftherm(ir+3) - galpha(ir)*deltat*((zk**2)-(zk1**2))/2.0D0
   END DO
...
!     CALCULATE CONTRIBUTION FROM SYMMETRIC LAYERS
   ...
   DO  ir = 1,3
     ...
     ftherm(ir+3) = ftherm(ir+3) - galpha(ir)*deltat*((zk1**2)-(zk**2))/2.0D0
   END DO
