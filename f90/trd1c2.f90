...
!     THIS ROUTINE IS SUITABLE FOR DOUBLE PRECISION OPERATION
 ...
 LOGICAL :: nopd
 INTEGER :: dit1,pnl1,pnl, sysbuf,FILE,iz(1), &
!RLBR SPR94003 9/94
!RLBR1                MCB(7),IPNL(7),SUBNAM(2)  &
             mcb(7),ipnl(7),subnam(2),moutpu(7)
...
!     CALL CLOSE (ISCR3,1)
 ...
 CALL CLOSE (iscr4,1)
 CALL wrttrl (mcb)
 IF (nlftp == 0 .OR. ispnl == 0) GO TO 190
 CALL CLOSE (pnl,j)
 CALL wrttrl (ipnl)
 190 RETURN
