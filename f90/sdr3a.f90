...
!     SORT-2  MODULE
 ...
 INTEGER :: iname(2),trail(7),id(146),idtemp(146),vector(50),  &
     ...
     ,               recpt,eor,outrwd,rwd,full,ovrlap,v_in_bk,w_per_bk,  &
     v_per_bk,total1,total2,ahead,entrys(85)
...
!                V IN  BK = VECTORS NOW IN ENTRY BLOCKS
   ...
   w_per_bk = icore/entrys(1)
   v_per_bk = w_per_bk/nwds
   w_per_bk = v_per_bk*nwds
   IF (v_per_bk < 1) GO TO 690
   
!     DISTRIBUTE FIRST DATA BLOCK TO INCORE ENTRY BLOCKS (BOTTOM TO TOP)
   
   nentry = entrys(1)
   total1 = w_per_bk*entrys(1) + 1
   total2 = nwds*entrys(1) + 1
   DO  i = 1,nentry
     n1  = total1 - w_per_bk*i
     n2  = total2 - nwds    *i
     ihd = ihd2 - i
     z(ihd) = z(n2)
     z(n1 ) = id(5)
...
!     NOT ENOUGH ROOM THUS DUMP CORE ENTRY BLOCKS ONTO SCRATCH FILES
   ...
   DO  i = 1,nentry
     ...
     CALL WRITE (scrtch(nfile),z(npoint),w_per_bk,eor)
     npoint = npoint + w_per_bk
   END DO
   recs = recs + nentry
...
!     COPY INCORE VECTORS TO OUTFILE
     ...
     390 words  = v_in_bk*nwds
     npoint = w_per_bk*i - w_per_bk + 1
