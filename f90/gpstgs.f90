...
! INSURE THE SYMMETRY OF THE B MATRIX
 ...
 IF (b(2) /= 0.0 .AND. b(4) /= 0.0) GO TO 40
 b(2) = 0.0
 b(4) = 0.0
 GO TO 50
 40 temp = (b(2) + b(4)) / 2.0
 b(2) = temp
 b(4) = temp
 50 IF (b(3) /= 0.0 .AND. b(7) /= 0.0) GO TO 60
 b(3) = 0.0
 b(7) = 0.0
 GO TO 70
 60 temp = (b(3) + b(7)) / 2.0
 b(3) = temp
 b(7) = temp
 70 IF (b(6) /= 0.0 .AND. b(8) /= 0.0) GO TO 80
 b(6) = 0.0
 b(8) = 0.0
 GO TO 90
 80 temp = (b(6) + b(8)) / 2.0
...
! HERE WE HAVE THAT THE ORDER OF THE SINGULARITY IS 2.
 ...
 inc2 = 2
 inc3 = 1
 220 IF (b(k1) <= 0.0  .AND.  b(k2) <= 0.0) GO TO 430
 IF (b(k1) <= 0.0) GO TO 230
 nwds      = 2
 ttlwds     = 4
 iarray(3) = ip + inc1
 iarray(4) = ip + inc2
