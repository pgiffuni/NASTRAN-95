SUBROUTINE alg08
 ...
 DIMENSION xx1(21),dladm(21),dsdm(21),drvwdm(21),dl(21),dsdl(21),&
     dphidl(21),fx1(21),fx2(21),vvold(21),afun(20),bfun(20),hs(20),&
     xm2(20),vmmax(21),dvmdvm(20),teip1(21),tbip1(21)
 ...
 DO  j=1,nstrms
   ...
   IF(x1 > 1.0)GO TO 426
   IF(ipass <= nforce)GO TO 424
   CALL alg03(lnct,1)
   WRITE(log2,422)ipass,i,j,x1
   422   FORMAT(5X,4HPASS,i3,9H  station,i3,12H  streamline,i3,40H  limiting &
                              &meridional velocity squared =,e12.5)
   424   x1=6250000.0
 ...
 DO  j=1,itub
   ...
   xq=x1
   IF(x1 <= 0.9801)GO TO 560
   IF(ipass <= nforce)GO TO 550
   IF(lnct < npage)GO TO 530
   WRITE(log2,500)
   lnct=1
   530   lnct=lnct+1
   x1=SQRT(x1)
   ...
   GO TO 562
   570   bfun(j)=bfun(j)+x7*(dsdm(j)+dsdm(j+1))*x3*(1.0-x1*(x6-1.0))/(1.0-x1)
 END DO
 ...
 jj=jold
 IF(jinc == -1)jj=j
 IF(ABS(afun(jj)) <= 1.0E-5) GO TO 660
 ...
 IF(x1 <= 0.9801)GO TO 910
 IF(ipass <= nforce)GO TO 900
 IF(lnct < npage)GO TO 880
 WRITE(log2,500)
 lnct=1
 880   lnct=lnct+1
 x1=SQRT(x1)
 ...
 iconf2=1
 950   vv(j)=SQRT(x1)
 IF(vv(j) <= vmmax(j))GO TO 958
 ifaie=1
 iconf2=1
 IF(ipass <= nforce)GO TO 956
 CALL alg03(lnct,1)
 WRITE(log2,706)ipass,i,iter,j,vv(j),vmmax(j)
 956   vv(j)=vmmax(j)
 958   IF(ABS(vv(j)/vold-1.0) <= tolnce*0.2)GO TO 990
 ...
 1234  DO  j=k,nstrms
   vv(j)=vv(j)+dv
   IF(vv(j) <= vmmax(j))GO TO 1238
   ifail=1
   vv(j)=vmmax(j)
   1238  IF(vv(j) >= 1.0)CYCLE
