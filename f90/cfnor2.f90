SUBROUTINE cfnor2 (right,left,size2,option,ri)
    ...
    DOUBLE PRECISION, INTENT(IN OUT)         :: right(1)
    DOUBLE PRECISION, INTENT(IN OUT)         :: left(1)
    INTEGER, INTENT(IN)                      :: size2
    INTEGER, INTENT(IN OUT)                  :: option
    DOUBLE PRECISION, INTENT(OUT)            :: ri(2)
    LOGICAL :: skip     ,qpr
 
    DOUBLE PRECISION :: rsqrt    , theta2   ,rj(2)
...
300 FORMAT (3H --,32(4H----), /,7H cfnor2,26X,  &
        16HOUTPUT magnitude,d16.8,8X,2D16.8, /,3H --,32(4H----))
