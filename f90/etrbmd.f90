...
!     ECPT(25) = ELEMENT TEMPERATURE                   ELTEMP    REAL
 ...
 DOUBLE PRECISION :: d(9),g(9),g2x2(4),j2x2(4),s(18),hyq(6),siij(7,7),  &
     mbaraa(9),mar(18),mrr(36),a,prod9,temp9,xsubb,  &
     xsubc,ysubc,bfact,e,aout,determ,temp,xcsq,ycsq,  &
     xbsq,xcyc,yprodj,fj,fj2,aij,bij,xprodi,fi,fij, sizero
 ...
 EQUIVALENCE      (ielid,ecpt(1),necpt(1)),(j2x2(1),a(14)),  &
     (d(1),g(1),a(1),siij(1,1)),(g2x2(1),a(10)),  &
...
!     GET THE G2X2 MATRIX
 ...
 inflag = 3
 CALL mat (ecpt(1))
 IF (g2x211 == 0.0 .AND. g2x212 == 0.0 .AND. g2x222 == 0.0) GO TO 410
...
!       RR
 ...
 mrr(22) = mrr(22) + hyq(1)*(hyq(1)*siij(3,1) + 2.0D0*(siij(5,1) +  &
     hyq(4)*siij(2,2))) + hyq(4)*(2.0D0*siij(4,2) + hyq(4)*siij(1,3))
 ...
 mrr(29) = mrr(29) + hyq(2)*(hyq(2)*siij(3,1) + 2.0D0*(siij(3,3) +  &
     hyq(5)*siij(2,2))) + hyq(5)*(2.0D0*siij(2,4) + hyq(5)*siij(1,3))
 ...
 mrr(36) = mrr(36) + hyq(3)*(hyq(3)*siij(3,1) + 2.0D0*(siij(2,4) +  &
     hyq(6)*siij(2,2))) + hyq(6)*(2.0D0*siij(1,5) + hyq(6)*siij(1,3))
